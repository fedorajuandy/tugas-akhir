    ...
    def __call__(
        ...
        if attention_mask is not None and self.causal:
            attention_mask = jnp.broadcast_to(
                jnp.expand_dims(attention_mask, axis=(-3, -2)), causal_mask.shape # KAGGLE
            )
...
class FlaxBartDecoder(nn.Module):
    ...
    def __call__(
        ...
        outputs = self.layers(
            ...
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict, # KAGGLE
        )
...
class FlaxBartForConditionalGenerationModule(FlaxBartForConditionalGenerationModule):
    ...
    def __call__(
        ...
        outputs = self.model(
            ...
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
            deterministic=deterministic, # KAGGLE
        )
